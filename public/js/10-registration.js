...
function validate(username, house, section, theme, consent) {
  ...
  if (username.trim().length > 24) return "Username must be 24 characters or less.";
  if (!house) return "Please choose a Hogwarts house.";
  if (!section) return "Please choose your preferred section.";
  if (!theme) return "Please choose a default theme mode.";
  if (!consent) return "You must agree to save your profile on this device.";
  return "";
}
